...
func TestRenderClaim(t *testing.T) {
	if claimFile == "" {
		fmt.Println("Usage: go test ./functions/ip-inject-fn -run TestRenderClaim -v -claim-file=claim.yaml")
			...
			"template": map[string]any{
				...
				"spec": map[string]any{
					"domain": map[string]any{
						...
						"networks": []any{
							map[string]any{"name": "external", "multus": map[string]any{"networkName": namespace + "/" + networkName}},
						},
					},
					"volumes": []any{
	...
	f := &Function{
		...
		testAllocateIP: func(ctx context.Context, vmName, subnet, networkView, ipPool string) (string, string, error) {
	...
	// Print XR status
	if xr := desired.GetComposite(); xr != nil {
		xrObj := xr.GetResource()
		if xrObj != nil {
			b, _ := json.Marshal(xrObj.AsMap())
			var raw any
			json.Unmarshal(b, &raw)
			yamlBytes, _ := yaml.Marshal(raw)
			fmt.Println("\n=== XR STATUS ===")
